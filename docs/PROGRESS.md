...
**Follow-up (2026-09-20) — real-world testing via Swagger surfaced a genuine observability gap:**
- Manually exercising the `/execute` endpoint against a real (non-existent-parameter-supporting) model produced a `422` with only the generic `"the request was rejected by the model provider"` message — the actual provider reason (`"Unsupported parameter: 'temperature' is not supported with this model."`, including which exact parameter, via OpenAI's `APIError.param`) was being discarded entirely once translated into the public HTTP response, per Step 3's original (correct) decision to never leak raw provider text externally. That decision protected the external API contract but left no internal way to diagnose an `invalid_request` failure beyond its bare category — confirmed by having to add a temporary `print(str(exc))` to actually see the real cause.
